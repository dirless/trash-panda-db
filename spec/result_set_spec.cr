...
describe TrashPandaDB::ResultSet do
  ...
  it "reads time stored in text fields, too" do
    with_db do |db|
      ...
      timestamp = Time.utc
      db.exec "INSERT INTO test_table (test_date) values (?)", timestamp.to_s TrashPandaDB::DATE_FORMAT_SUBSECOND
      db.exec "INSERT INTO test_table (test_date) values (?)", timestamp.to_s TrashPandaDB::DATE_FORMAT_SECOND
      db.exec "INSERT INTO test_table (test_date) values (current_timestamp)"
      db.query("SELECT test_date FROM test_table") do |rs|
        rs.each do
          rs.read(Time).should be_close(timestamp, 1.second)
