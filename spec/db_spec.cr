...
DB::DriverSpecs(TrashPandaDB::Any).run do |ctx|
  ...
  connection_string "trashpanda:#{DB_FILENAME}"
  ...
  sample_value Bytes.new(ary.to_unsafe, ary.size), "blob", "X'53514C697465'"
  ...
  pending "raises on unsupported param types" do
    # SQLite3-specific error message format; trashpanda will have its own
  end
  ...
  pending "handles single-step pragma statements" do
    # PRAGMA is SQLite3-specific
  end

  pending "handles multi-step pragma statements" do
    # PRAGMA is SQLite3-specific
  end

  pending "handles REGEXP operator" do
    # REGEXP is SQLite3-specific
  end
end
